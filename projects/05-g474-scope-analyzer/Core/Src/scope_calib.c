/**
 * @file    scope_calib.c
 * @brief   信号链路校准实现 — 2026-08-01 滤波器移除后重新标定
 * @author  jiujiu0415
 * @date    2026-08-01
 *
 * 校准数据来源 (2026-08-01 重测, 滤波器已移除):
 *   信号发生器 → 加法器 → AD603 AGC → 直流偏置 → ADC
 *   11 个输入电平 (50-250 mVpp) × 7 个频率 (10k-500k Hz) = 77 点
 ...
 *
 *  模型 (M3, 最优 — 最小化 V_original 还原误差):
 *    G_total = a0 + a1·Vd + a2·Vd² + a3·fn + a4·fn·Vd
 *    fn = (freq_hz − 10000) / 490000  ∈ [0, 1]
 *
 *  拟合精度 (77点实测):
 *    R² = 0.9984, G_MAE = 0.43, G_MaxE = 2.11
 *    V_original 还原: MAE = 1.82%, MaxE = 4.60% (@500kHz, 70mVpp 边界)
 *
 *  G 范围: [11.7, 61.3]  (Vd ∈ [522, 718] mV, f ∈ [10k, 500k] Hz)
 *  fn ∈ [0, 1] 归一化保证 float32 下系数为 O(1), 无精度损失
 *
 *  物理含义:
 *    a0+a1·Vd+a2·Vd² — 基础增益曲线 (Vd 控制 AGC)
 *    a3·fn            — 纯频率偏移 (检波器频响)
 *    a4·fn·Vd         — 频率×幅度交叉项 (检波器效率同时依赖频率和幅度)
 * ================================================================ */
...
float ScopeAGC_ComputeGain(float vd_mV, float freq_hz)
{
    ...
    /* Horner 形式: a0 + Vd*(a1 + a2*Vd) + fn*(a3 + a4*Vd) */
